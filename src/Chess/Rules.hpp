...
//! chess engines.
enum class Status { Playing, WhiteWon, BlackWon, Stalemate, NoMoveAvailable, /*FIXME a separer*/ InternalError };
...
public:
  ...
  Rules(chessboard const& board, Color const side = Color::White,
        bool const noking = WithKings, Castle const wcastle = Castle::Both,
        Castle const bcastle = Castle::Both, Square const ep = OOB);
  ...
  bool isKingInCheck(chessboard const& board, Color const side) const;
...
private:
  ...
  void generatePseudoLegalPawnMove(uint8_t const from, PieceType const pt);

  //! \brief Generate a list of pseudo legal of pieces moves.
  void generatePseudoLegalPieceMove(uint8_t const from, PieceType const pt);
  ...
  //! legal move or not.
  bool tryMove(Move const move) const;

  //! \brief From a given position, check if the square sq is attacked by a piece of the
  //! opponent side of side.
  bool attack(chessboard const& position, uint8_t const sq, Color const side) const;
